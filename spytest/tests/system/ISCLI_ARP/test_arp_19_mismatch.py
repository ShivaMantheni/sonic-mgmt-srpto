"""
Test ID: 4.17.19
Test Case: Verify ARP Reply with Mismatched IP/MAC
Test Item: Negative
Test Objective: Validate system handling when Ethernet source MAC != ARP hardware source MAC
"""
...
CONFIG.dut1_ip = "10.19.1.1/24"
CONFIG.dut2_ip = "10.19.1.2/24"
CONFIG.mismatched_mac = "aa:bb:cc:dd:ee:ff"
CONFIG.script_file = "/tmp/scenario19_mismatch.py"


@pytest.fixture(scope="module", autouse=True)
def arp_mismatch_module_hooks(request):
    global vars, data
...
def test_arp_mismatch():
    """Test ID: 4.17.19 - Expected: Mismatched Ether src != ARP hwsrc should be dropped/detected"""
    result = True
    try:
        st.log("Test: ARP Reply with Mismatched IP/MAC")
...
ether_src = "{dut2_mac}"
arp_hwsrc = "{CONFIG.mismatched_mac}"
src_ip = "{dut2_ip}"
dst_ip = "{dut1_ip}"

print("[*] Scenario 19: ARP Reply with Mismatched IP/MAC")
print(f"[*] Ether src: {{ether_src}}, ARP hwsrc: {{arp_hwsrc}}")

mismatched_arp = Ether(src=ether_src, dst="ff:ff:ff:ff:ff:ff") / ARP(
    op=2,
    hwsrc=arp_hwsrc,
    psrc=src_ip,
...
sendp(mismatched_arp, iface=iface, verbose=True, count=5)
print("[+] Mismatched ARP reply sent")
"""
