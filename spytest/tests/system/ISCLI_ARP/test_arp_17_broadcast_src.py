"""
Test ID: 4.17.17
Test Case: Verify ARP with Broadcast Source MAC
Test Item: Negative
Test Objective: Validate system handling of invalid ARP with broadcast source MAC
"""
...
CONFIG.dut1_ip = "10.17.1.1/24"
CONFIG.dut2_ip = "10.17.1.2/24"
CONFIG.broadcast_mac = "ff:ff:ff:ff:ff:ff"
CONFIG.script_file = "/tmp/scenario17_broadcast_src.py"


@pytest.fixture(scope="module", autouse=True)
def arp_broadcast_src_module_hooks(request):
    global vars, data
...
def test_arp_broadcast_src():
    """Test ID: 4.17.17 - Expected: Invalid broadcast source MAC should be dropped"""
    result = True
    try:
        st.log("Test: ARP with Broadcast Source MAC")
...
src_mac = "{CONFIG.broadcast_mac}"
src_ip = "{dut1_ip}"
dst_ip = "{dut2_ip}"

print("[*] Scenario 17: ARP with Broadcast Source MAC")
print(f"[*] Sending ARP with invalid broadcast source MAC")

invalid_arp = Ether(src=src_mac, dst="ff:ff:ff:ff:ff:ff") / ARP(
    op=1,
    hwsrc=src_mac,
    psrc=src_ip,
...
sendp(invalid_arp, iface=iface, verbose=True, count=5)
print("[+] Invalid ARP sent")
"""
