"""
TC_BREAKOUT_02: Basic 800G to 4x200G Breakout

Test Case ID: OC-BREAKOUT-02
...
How to run:
  ...
  ./bin/spytest --tryssh 1 \
    ...
    tests/system/OC_Port_Breakout/test_oc_breakout_02_4x200g.py \
    --logs-path ./logs/oc_breakout_02_$(date +%F_%H%M%S) \
    --log-level debug --skip-init-config --ifname-type native

Description:
  Validates basic port breakout from 1x800G to 4x200G.

  Test Objective:
  - Verify port breakout from default 1x800G mode to 4x200G mode
  - Verify all 4 child ports are created (Ethernet0, 2, 4, 6)
  ...
  2. Apply breakout: Ethernet0 -> 4x200G
  3. Verify breakout success message
  4. Verify 'show interface breakout' displays 4x200G for Ethernet0
  5. Verify 'show interface status' shows all 4 child ports (Ethernet0,2,4,6)
  6. Verify each child port shows 200GB speed
  ...
  - 'show interface breakout' displays "Ethernet0    4x200G    Configured"
  - All 4 child ports visible in 'show interface status'
  - Each child port shows speed as 200GB
...
Pre-requisites:
  ...
  - Port Ethernet0 must support 4x200G breakout mode
...
CONFIG = SpyTestDict({
    ...
    # Expected child ports after 4x200G breakout
    # 4 ports with 2-lane spacing (uses 2 lanes per port)
    "expected_child_ports": [
        "Ethernet0", "Ethernet2", "Ethernet4", "Ethernet6"
    ],
    
    # Expected values
    "expected_num_children": 4,
    "expected_speed": "200GB",
    "expected_breakout_mode": "4x200G",
})

# Test Case IDs
TC_IDS = SpyTestDict({
    "oc_breakout_02": "OC-BREAKOUT-02",
})


@pytest.fixture(scope="module", autouse=True)
def oc_breakout_02_module_hooks(request):
    """Module-level setup and teardown."""
    global vars, data

    st.banner("=" * 80)
    st.banner("TC_BREAKOUT_02 MODULE CONFIGURATION - START")
    st.banner("=" * 80)
    ...
    oc_breakout_02_pre_config()

    st.banner("=" * 80)
    st.banner("TC_BREAKOUT_02 MODULE CONFIGURATION - COMPLETE")
    st.banner("=" * 80)

    yield

    st.banner("=" * 80)
    st.banner("TC_BREAKOUT_02 MODULE CLEANUP - START")
    st.banner("=" * 80)

    oc_breakout_02_cleanup()

    st.banner("=" * 80)
    st.banner("TC_BREAKOUT_02 MODULE CLEANUP - COMPLETE")
    st.banner("=" * 80)


def oc_breakout_02_pre_config():
...
def apply_breakout_4x200g(dut: str) -> bool:
    """
    Apply 4x200G breakout to test port.
...
def verify_breakout_configuration(dut: str) -> bool:
    ...
    try:
        ...
        # Validation 2: Check 4x200G mode is displayed
        # Look for pattern: Ethernet0    4x200G    Configured
...
def verify_child_ports_status(dut: str) -> bool:
    """
    Verify all 4 child ports are visible in 'show interface status'.
    ...
    try:
        ...
        # Validation 1: Check all 4 child ports exist
        found_ports = []
        missing_ports = []
        ...
        # Validation 2: Check speed is 200GB for child ports
        # Sample line: Ethernet0    -    up    200GB    9100
        speed_verified = 0
        for port in CONFIG.expected_child_ports:
            # Look for pattern with 200GB
...
def test_oc_breakout_02_4x200g():
    """
    TC_BREAKOUT_02: Basic 800G to 4x200G Breakout
    
    Test Steps:
    1. Apply breakout: Ethernet0 -> 4x200G
    2. Verify breakout success message
    3. Verify 'show interface breakout' displays 4x200G
    4. Verify all 4 child ports visible in 'show interface status'
    5. Verify child ports show 200GB speed
    6. Revert breakout to 1x800G
    
    Pass Criteria:
    - Breakout succeeds with success message
    - 'show interface breakout' displays 4x200G for Ethernet0
    - All 4 child ports (Ethernet0,2,4,6) are visible
    - Ports show 200GB speed
    - Revert succeeds
    """
    st.banner("=" * 80)
    st.banner("TC_BREAKOUT_02: Basic 800G to 4x200G Breakout TEST - START")
    st.banner("=" * 80)

    validation_errors = []

    # ============================================================
    # STEP 1: Apply 4x200G Breakout
    # ============================================================
    st.banner("STEP 1: Apply 4x200G Breakout")
    
    if not apply_breakout_4x200g(vars.D1):
        validation_errors.append("Breakout application failed")
        st.generate_tech_support([vars.D1], "oc_breakout_02_apply_failed")
        st.report_tc_fail(TC_IDS.oc_breakout_02, "msg", "Breakout application failed")
    else:
    ...
    if not verify_breakout_configuration(vars.D1):
        ...
        st.generate_tech_support([vars.D1], "oc_breakout_02_verify_failed")
    else:
    ...
    if not verify_child_ports_status(vars.D1):
        ...
        st.generate_tech_support([vars.D1], "oc_breakout_02_ports_failed")
    else:
    ...
    # ============================================================
    st.banner("=" * 80)
    st.banner("TC_BREAKOUT_02 TEST - COMPLETE")
    st.banner("=" * 80)

    if validation_errors:
        error_summary = f"TC_BREAKOUT_02 FAILED: {'; '.join(validation_errors)}"
        st.error(error_summary)
        st.banner("TEST RESULT: FAILED ❌")
        st.report_fail("test_case_failed", error_summary)
    else:
        success_msg = "TC_BREAKOUT_02 PASSED: 4x200G breakout successful"
        st.log(success_msg)
        st.banner("TEST RESULT: PASSED ✅")
        st.log("=" * 80)
        st.log("TEST SUMMARY - TC_BREAKOUT_02")
        st.log("=" * 80)
