...
def test_bgp55_ibgp_ebgp_selection():
    ...
    try:
        ...
        if not configure_routemap(vars.D1, CONFIG.rm_ibgp, CONFIG.local_pref):
            error_msg = f"Route-map {CONFIG.rm_ibgp} configuration failed on {vars.D1}"
            st.error(error_msg)
            validation_failures.append(error_msg)

        if not configure_routemap(vars.D1, CONFIG.rm_ebgp, CONFIG.local_pref):
            error_msg = f"Route-map {CONFIG.rm_ebgp} configuration failed on {vars.D1}"
            st.error(error_msg)
            validation_failures.append(error_msg)
        ...
        if not configure_bgp_basic(vars.D1, CONFIG.dut1_asn, CONFIG.dut1_router_id):
            error_msg = f"BGP AS {CONFIG.dut1_asn} configuration failed on {vars.D1}"
            st.error(error_msg)
            validation_failures.append(error_msg)

        if not configure_bgp_basic(vars.D2, CONFIG.dut2_asn_ibgp, CONFIG.dut2_router_id):
            error_msg = f"BGP AS {CONFIG.dut2_asn_ibgp} configuration failed on {vars.D2}"
            st.error(error_msg)
            validation_failures.append(error_msg)
        ...
        if not attach_neighbor_with_routemap(vars.D1, CONFIG.dut2_ip,
            ...
            error_msg = f"IBGP neighbor configuration failed on {vars.D1}"
            st.error(error_msg)
            validation_failures.append(error_msg)

        if not attach_neighbor_with_routemap(vars.D2, CONFIG.dut1_ip,
                                             CONFIG.dut1_asn, CONFIG.rm_ibgp):
            error_msg = f"IBGP neighbor configuration failed on {vars.D2}"
            st.error(error_msg)
            validation_failures.append(error_msg)
        ...
        if not change_bgp_as(vars.D2, CONFIG.dut2_asn_ibgp, CONFIG.dut2_asn_ebgp,
            ...
            error_msg = f"BGP AS change from {CONFIG.dut2_asn_ibgp} to {CONFIG.dut2_asn_ebgp} failed on {vars.D2}"
            st.error(error_msg)
            validation_failures.append(error_msg)
        ...
        if not attach_neighbor_with_routemap(vars.D2, CONFIG.dut1_ip,
            ...
            error_msg = f"EBGP neighbor configuration failed on {vars.D2}"
            st.error(error_msg)
            validation_failures.append(error_msg)
        ...
        if not verify_ebgp_preference(vars.D1, CONFIG.test_prefix):
            st.log(f"Warning: EBGP route preference verification inconclusive for {CONFIG.test_prefix}")
