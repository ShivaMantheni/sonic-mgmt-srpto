# L2-03: Deny Exact Destination MAC - Manual Test Log
...
### 2.1 Create L2 ACL with Deny Destination MAC Rule
...
# Create L2 ACL for destination MAC address deny rule
# MAC address format: 00:BB:BB:BB:BB:02 (RX host MAC - will be DENIED)
mac access-list L2_ACL_TEST_DEST_DENY

# Rule 1: Deny traffic to specific destination MAC
deny any host 00:bb:bb:bb:bb:02
...
interface Ethernet40
mac access-group L2_ACL_TEST_DEST_DENY in
exit
...
```bash
# Command executed
show access-list L2_ACL_TEST_DEST_DENY

# Expected output:
mac access-list L2_ACL_TEST_DEST_DENY
 10 deny any host 00:bb:bb:bb:bb:02
 20 permit any any
```
...
sudo nohup tcpdump -i Ethernet40 'ether dst 00:bb:bb:bb:bb:02' -w /tmp/l2_03_test.pcap -c 20 > /dev/null 2>&1 &
...
cat > /tmp/l2_03_traffic.py << 'EOF'
#!/usr/bin/env python3
"""
L2-03: Deny Exact Destination MAC Test
Sends 10 packets to destination MAC (00:BB:BB:BB:BB:02) - ALL SHOULD BE BLOCKED
"""
...
print(f"[+] L2-03: Deny Exact Destination MAC Test")
...
```bash
# Run traffic script
sudo python3 /tmp/l2_03_traffic.py
...
# Expected output (empty since packets were denied):
-rw-r--r-- 1 root root 24 Mar 18 10:15 /tmp/l2_03_test.pcap
...
show access-list L2_ACL_TEST_DEST_DENY statistics

# Expected output:
MAC ACL L2_ACL_TEST_DEST_DENY:
  Rule 10 (deny any host dst):
    Matched packets: 10
    Matched octets: 1024
  Rule 20 (permit):
    Matched packets: 0
...
interface Ethernet40
no mac access-group L2_ACL_TEST_DEST_DENY in
exit

# Delete ACL
no mac access-list L2_ACL_TEST_DEST_DENY
exit
...
# Expected output:
% List L2_ACL_TEST_DEST_DENY not found
```
...
| **Pass Criteria** | ✓ Deny destination MAC rule working correctly |
...
| Destination MAC Match | Exact 00:bb:bb:bb:bb:02 | ✓ Confirmed | ✓ PASS |
...
2. **Traffic Blocking**: All 10 packets destined to specific MAC were successfully blocked
3. **ACL Behavior**: Deny rule correctly prevented forwarding to specified destination
...
The L2-03 test case demonstrates that L2 ACL deny rules work correctly for exact destination MAC address matching. Traffic destined to the specified MAC (00:bb:bb:bb:bb:02) is correctly blocked and not forwarded through the DUT's L2 switching pipeline, with 0% delivery rate as expected.
