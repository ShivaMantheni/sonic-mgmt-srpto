...
Description:
  ...
  Steps:
    ...
    6. Verify packet received on access port
...
class TestVlanEgressTaggedPacketOnAccessPort:
    ...
    def setup_class(cls) -> None:
        ...
        cls.data.dut2_port = topology.D2D1P1  # Access port on D2
        ...
        st.log(f"DUT2 Port (Access): {cls.data.dut2_port}")
    ...
    def _send_tagged_packet(self, dut, src_interface: str, src_mac: str, dst_mac: str,
        ...
        try:
            ...
            script_lines = [
                ...
                f"      Raw(load=\\\"TaggedPacketTestPayload\\\")",
                "",
                "try:",
            ...
            script_path = f"/tmp/scapy_tagged_packet_{int(time.time())}.py"
            script_content = "\\n".join(script_lines)
    ...
    def test_vlan_tag_002_egress_tagged_packet_on_access_port(self) -> None:
        """
        TC_VLAN_TAG_002: Egress Tagged Packet on Access Port Test

        Test execution with step tracking:
        1. Create VLAN 10 on both DUTs
        2. Configure trunk and access ports
        3. Get MAC addresses
        4. Send VLAN tagged packet from trunk port
        5. Verify traffic on access port
        """
        st.banner("=" * 100)
        st.banner("TC_VLAN_TAG_002: EGRESS TAGGED PACKET ON ACCESS PORT - EXECUTION")
        st.banner("=" * 100)

        test_results = {
            "step_1": False,  # Create VLAN on both DUTs
            "step_2": False,  # Configure trunk and access ports
            "step_3": False,  # Get MAC addresses
            "step_4": False,  # Send tagged packet from trunk port
            "step_5": False,  # Verify traffic on access port
        }
        ...
        try:
            ...
            if dut1_ok and dut2_ok:
                ...
                st.log(f"❌ STEP 1 FAIL: Could not create VLAN {vlan_id} on one or both DUTs")
                test_results["step_1"] = False

            # STEP 2: Configure trunk and access ports
            st.banner("STEP 2: Configuring trunk and access ports")
            all_ok = True

            # Configure DUT1 port as tagged trunk port
            try:
                st.log(f"  Configuring DUT1 port {self.data.dut1_port} as tagged trunk port")
            ...
            # Configure DUT2 port as untagged access port
            try:
                st.log(f"  Configuring DUT2 port {self.data.dut2_port} as untagged access port")
                vlan_api.add_vlan_member(
                    self.data.dut2, vlan_id, self.data.dut2_port, tagging_mode="untagged",
                    cli_type=self.data.cli_type, skip_error_check=True
                )
                st.log(f"    ✅ DUT2 access port configured")
            except Exception as e:
                st.log(f"    ❌ DUT2 access port configuration failed: {e}")
                all_ok = False

            if all_ok:
                st.log(f"✅ STEP 2 PASS: Ports configured")
            ...
            if mac_dut1 and mac_dut2:
                ...
                st.log(f"  DUT1 Port MAC: {mac_dut1}")
                st.log(f"  DUT2 Port MAC: {mac_dut2}")
            ...
            # STEP 4: Send VLAN tagged packet from trunk port
            st.banner("STEP 4: Sending VLAN tagged packet from trunk port")
            ...
            # STEP 5: Verify traffic on access port
            st.banner("STEP 5: Verifying traffic received on access port")
            if self._verify_traffic_on_ports(self.data.dut2, [self.data.dut2_port]):
                st.log(f"✅ STEP 5 PASS: Traffic verified on access port")
