# LLDP Test Case 4.16.15: TTL and Hold Time Behavior

## Test Information

**Test ID:** 4.16.15
**Test Name:** ttl_hold_behavior
**Feature:** LLDP (Link Layer Discovery Protocol)
**Test Result:** ✗ **FAIL - TTL not shown, global disable ineffective**

---

## Test Objective

Verify TTL value is shown and global LLDP disable works

---

## Test Configuration

### Configuration Steps:

```bash
show lldp neighbor Ethernet 8
no lldp enable
```

---

## Expected Result

TTL should be visible, neighbors should be withdrawn after global disable

---

## Actual Result

TTL value not shown, neighbors remain visible after global disable

---

## Test Status

**Status:** FAIL

### Known Issues

BUG: TTL not displayed, global disable ineffective


---

## Test Execution

### Prerequisites:
- Two DUTs connected back-to-back
- LLDP feature available in SONiC
- sonic-cli (ISCLI) access to both DUTs

### Test Steps:

1. Configure LLDP as per configuration steps above
2. Verify configuration using show commands
3. Check LLDP neighbor discovery and TLV information
4. Validate expected behavior against actual behavior

### Verification Commands:

```bash
show lldp neighbor
show lldp neighbor <interface>
show lldp local <interface>
show lldp configuration
show lldp statistics
```

---

## Related Test Cases

- Test 4.16.1: Global and Interface LLDP CLI Configuration
- Test 4.16.2: LLDP Neighbor Discovery
- Test 4.16.10: LLDP Configuration Persistence

---

## References

- IEEE 802.1AB LLDP Standard
- SONiC LLDP Feature Documentation
- OC-1 Manual Testing Document

---

**Test Script:** `test_lldp_15_ttl_hold_behavior.py`
**Documentation:** `test_lldp_15_ttl_hold_behavior.md`
**Test Suite:** iscli_LLDP
**Framework:** spytest
