...
# Runs all NTP tests and generates a feature-specific
# dashboard + JSON summary under this folder's reports/ dir.
#
# Outputs (all under tests/automation/ntp/reports/):
#   reports/results/<batch>/      -> per-batch spytest logs (single dir, wiped each run)
#   reports/ntp_dashboard.html    -> graphical dashboard
#   reports/NTP_summary.json      -> JSON built from results_*_functions.csv
# ==========================================================
...
# Resolve paths.
#   SCRIPT_DIR   = .../spytest/tests/automation/ntp
...
DASHBOARD_FILE="${REPORTS_DIR}/ntp_dashboard.html"
JSON_FILE="${REPORTS_DIR}/NTP_summary.json"
...
# ==========================================================
# Batch 1/2 : NTP iSCLI (testbed_vs_1node_reg.yaml) - 1 test
# Requires a local NTP server. Set it up + verify before the run, and point the
# test at the local-server vars file via NTP_ISCLI_VAR_FILE (env consumed by the
# test). Setup is best-effort: failures are warned, not fatal.
# ==========================================================
echo "Batch 1/2: NTP iSCLI (with local NTP server setup)"

# setup_ntp_server.sh requires root (installs chrony, writes /etc/chrony,
# restarts the service). To keep this seamless:
#   1) If chrony is already up, skip setup entirely (no sudo needed).
#   2) Otherwise run it via non-interactive sudo (sudo -n) so it never prompts.
#      This needs passwordless sudo for these scripts; configure once with:
#        echo "$(whoami) ALL=(root) NOPASSWD: ${SCRIPT_DIR}/scripts/setup_ntp_server.sh, ${SCRIPT_DIR}/scripts/verify_ntp_server.sh, ${SCRIPT_DIR}/scripts/fix_ntp_server.sh" | sudo tee /etc/sudoers.d/ntp_setup
#        sudo chmod 440 /etc/sudoers.d/ntp_setup
if systemctl is-active --quiet chrony 2>/dev/null; then
    echo "Local NTP server (chrony) already running — skipping setup."
else
    echo "Setting up local NTP server..."
    sudo -n "${SCRIPT_DIR}/scripts/setup_ntp_server.sh" \
        || echo "WARN: NTP setup needs root. Configure passwordless sudo (see comment above) or pre-run the setup once."
    echo "Verifying NTP server..."
    sudo -n "${SCRIPT_DIR}/scripts/verify_ntp_server.sh" || true
    sudo -n "${SCRIPT_DIR}/scripts/fix_ntp_server.sh" || true
fi

export NTP_ISCLI_VAR_FILE="${SCRIPT_DIR}/vars/vars_ntp_iscli_local.yaml"
echo "NTP_ISCLI_VAR_FILE=${NTP_ISCLI_VAR_FILE}"

run_ntp_test "NTP_ISCLI" "./testbeds/testbed_vs_1node_reg.yaml" \
    automation/ntp/scripts/test_ntp_iscli.py

# ==========================================================
# Batch 2/2 : NTP Extended Tests (testbed_vs_1node_ntp_reg.yaml) - 4 tests
# ==========================================================
echo "Batch 2/2: NTP Extended Tests"
run_ntp_test "NTP_EXTENDED" "./testbeds/testbed_vs_1node_ntp_reg.yaml" \
    automation/ntp/scripts/test_ntp_negative.py \
    automation/ntp/scripts/test_ntp_persistence.py \
    automation/ntp/scripts/test_ntp_traffic.py \
    automation/ntp/scripts/test_ntp_comprehensive.py

# ==========================================================
# Generate Feature JSON  -> reports/NTP_summary.json
...
# ==========================================================
# Generate Feature Dashboard  -> reports/ntp_dashboard.html
