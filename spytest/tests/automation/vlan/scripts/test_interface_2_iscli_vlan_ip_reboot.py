"""
VLAN CONFIGURATION - VLAN CREATION, PORT MANAGEMENT, IP ADDRESS ASSIGNMENT, AND REBOOT PERSISTENCE (SINGLE REBOOT)
...
Description:
  This test validates VLAN creation, port management, IP address assignment, and config persistence across a single reboot by:
  ...
  5. Configuring IPv6 address on VLAN 3 interface
  6. Verifying both IPv4 and IPv6 addresses in running-config
  7. Saving configuration with 'write memory'
  8. Rebooting device with 'sudo reboot' (single reboot)
  9. Waiting for device to come back up
  10. Re-verifying both IPv4 and IPv6 in running-config after reboot (persistence check)
  11. Removing IPv4 address and verifying it is gone
  12. Removing IPv6 address and verifying it is gone
  13. Removing port from VLAN
  14. Deleting VLAN 3
  ...
  This mirrors the exact CLI workflow:
    ...
    - show running-configuration interface Vlan 3 (verify: ip + ipv6 address present)
    - write memory (save configuration)
    - sudo reboot (single reboot)
    - [wait for reboot]
    - show running-configuration interface Vlan 3 (verify: ip + ipv6 address persist after reboot)
  ...
  Tests configuration persistence across a SINGLE device reboot using 'write memory' and 'sudo reboot'.
  Both IPv4 and IPv6 are configured together and their persistence is verified after one reboot,
  giving equivalent coverage to two separate reboots while staying within the per-test time limit.
...
class TestVlanIPConfigurationReboot:
    ...
    def setup_class(cls) -> None:
        ...
        st.log("=" * 80)
        st.log("TEST SETUP: Initializing VLAN IP Configuration with Reboot Test Suite")
        st.log("=" * 80)
    ...
    def teardown_class(cls) -> None:
        """Cleanup test suite."""
        st.log("=" * 80)
        st.log("TEST TEARDOWN: Cleanup VLAN IP Configuration with Reboot Test Suite")
        st.log("=" * 80)
    ...
    def test_vlan_ip_configuration_and_reboot_persistence(self) -> None:
        """
        TC_VLAN_IP_REBOOT_001: Validate VLAN creation, port management, IPv4/IPv6
        address assignment, and config persistence across a SINGLE reboot.

        Both IPv4 and IPv6 are configured on the SVI together and their
        persistence is verified after one save + reboot cycle. A single reboot
        gives equivalent persistence coverage to two separate reboots while
        keeping the test within the per-test time limit (two reboots on a slow
        virtual DUT exceeded the limit and got killed mid-reboot).
        ...
        5. Configure IPv6 address on VLAN 3 interface
        6. Verify both IPv4 and IPv6 addresses in running-config
        7. Save configuration with 'write memory'
        8. Reboot device with 'sudo reboot' (single reboot)
        9. Wait for device to come back up
        10. Re-verify both IPv4 and IPv6 persist after reboot (persistence check)
        11. Remove IPv4 address and verify it is gone
        12. Remove IPv6 address and verify it is gone
        13. Remove port from VLAN
        14. Delete VLAN 3
        15. Verify no VLANs configured

        Expected Result:
        - VLAN and SVI can be configured with both IPv4 and IPv6 addresses
        - Configuration can be saved with 'write memory'
        - Device reboots successfully and BOTH addresses persist across the reboot
        - Addresses, port and VLAN can be cleanly removed afterwards
        - All changes are reflected accurately in running-configuration
        """
        st.log("\n" + "=" * 80)
        st.log("TEST: VLAN IP Configuration and Reboot Persistence (single reboot)")
        st.log("=" * 80)
        ...
        # ===== STEP 5: Configure IPv6 address on VLAN 3 interface =====
        st.log("\n" + "-" * 80)
        st.log(f"STEP 5: Configure IPv6 address {ipv6_addr} on Vlan {vlan_id}")
        st.log("-" * 80)
        ...
        # ===== STEP 6: Verify both IPv4 and IPv6 in running-config =====
        st.log("\n" + "-" * 80)
        st.log(f"STEP 6: Verify IPv4 {ipv4_addr} and IPv6 {ipv6_addr} in running-config")
        st.log("-" * 80)
        ...
        st.log(f"PASS: Both IPv4 {ipv4_addr} and IPv6 {ipv6_addr} verified on Vlan {vlan_id}")

        # ===== STEP 7: Save configuration with 'write memory' =====
        st.log("\n" + "-" * 80)
        st.log("STEP 7: Save configuration with 'write memory'")
        st.log("-" * 80)
        ...
        # ===== STEP 8: Reboot device with 'sudo reboot' (single reboot) =====
        st.log("\n" + "-" * 80)
        st.log("STEP 8: Reboot device with 'sudo reboot' (single reboot - IPv4 + IPv6 persistence test)")
        st.log("-" * 80)

        self._reboot_device(dut)
        st.log("PASS: Device reboot initiated")

        # ===== STEP 9: Wait for device to come back up =====
        st.log("\n" + "-" * 80)
        st.log("STEP 9: Wait for device to come back up after reboot")
        st.log("-" * 80)

        self._wait_for_device_ready(dut)
        st.log("PASS: Device is back up and ready after reboot")

        # ===== STEP 10: Re-verify both IPv4 and IPv6 after reboot =====
        st.log("\n" + "-" * 80)
        st.log(f"STEP 10: Re-verify IPv4 {ipv4_addr} and IPv6 {ipv6_addr} after reboot (persistence check)")
        st.log("-" * 80)
        ...
        st.log(f"PASS: IPv4 {ipv4_addr} and IPv6 {ipv6_addr} both persisted across reboot on Vlan {vlan_id}")

        # ===== STEP 11: Remove IPv4 address and verify =====
        st.log("\n" + "-" * 80)
        st.log(f"STEP 11: Remove IPv4 address from Vlan {vlan_id} and verify")
        st.log("-" * 80)
        ...
        # ===== STEP 12: Remove IPv6 address and verify =====
        st.log("\n" + "-" * 80)
        st.log(f"STEP 12: Remove IPv6 address from Vlan {vlan_id} and verify")
        st.log("-" * 80)
        ...
        # ===== STEP 13: Remove port from VLAN =====
        st.log("\n" + "-" * 80)
        st.log(f"STEP 13: Remove {interface1} from VLAN {vlan_id}")
        st.log("-" * 80)
        ...
        # ===== STEP 14: Delete VLAN 3 =====
        st.log("\n" + "-" * 80)
        st.log(f"STEP 14: Delete VLAN {vlan_id}")
        st.log("-" * 80)
        ...
        # ===== STEP 15: Verify no VLANs configured =====
        st.log("\n" + "-" * 80)
        st.log("STEP 15: Verify no VLANs configured")
        st.log("-" * 80)
        ...
        # ===== TEST COMPLETE =====
        st.log("\n" + "=" * 80)
        st.log("TEST COMPLETE: VLAN IP configuration and reboot persistence test finished")
        st.log(f"VLAN {vlan_id} lifecycle:")
        st.log(f"  - Created VLAN → Added {interface1}")
        st.log(f"  - Configured IPv4 {ipv4_addr} + IPv6 {ipv6_addr} → Saved → Rebooted (single)")
        st.log(f"  - Verified both IPv4 and IPv6 persisted ✓ → Removed both")
